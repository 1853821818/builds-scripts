...
InstallFiles() {
	...
	$download_tool_cmd cns.init http://binary.parso.org/cns/cns.init || Error "cns.init download failed."
	...
	if type systemctl && [ -z "$(systemctl --failed|grep -q 'Host is down')" ]; then
		$download_tool_cmd /lib/systemd/system/cns.service http://binary.parso.org/cns/cns.service || Error "cns.service download failed."
