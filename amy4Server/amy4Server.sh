...
InstallFiles() {
	...
	download_tool amy4Server.init http://binary.parso.org/amy4Server/amy4Server.init || Error "amy4Server.init download failed."
	...
	if type systemctl &>/dev/null && [ -z "$(systemctl --failed|grep -q 'Host is down')" ]; then
		download_tool /lib/systemd/system/amy4Server.service http://binary.parso.org/amy4Server/amy4Server.service || Error "amy4Server.service download failed."
