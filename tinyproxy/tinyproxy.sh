...
InstallFiles() {
	...
	$download_tool_cmd tinyproxy https://raw.githubusercontent.com/1853821818/builds-scripts/main/tinyproxy/${tinyproxy_UPX}/linux_${machine} || Error "tinyproxy download failed."
	$download_tool_cmd tinyproxy.init https://raw.githubusercontent.com/1853821818/builds-scripts/main/tinyproxy/tinyproxy.init || Error "tinyproxy.init download failed."
	...
	if type systemctl &>/dev/null && [ -z "$(systemctl --failed|grep -q 'Host is down')" ]; then
		$download_tool_cmd /lib/systemd/system/tinyproxy.service https://raw.githubusercontent.com/1853821818/builds-scripts/main/tinyproxy/tinyproxy.service || Error "tinyproxy.service download failed."
...
InstallInit() {
	...
	type curl &>/dev/null && download_tool_cmd='curl -L -ko' || download_tool_cmd='wget --no-check-certificate -O'
}
...
Install() {
	...
	type systemctl &>/dev/null && [ -z "$(systemctl --failed|grep -q 'Host is down')" ] && systemctl restart tinyproxy &>/dev/null
	echo $echo_e_arg \
