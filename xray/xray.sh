...
InstallFile() {
	...
	#install xray
	$download_tool_cmd xray https://raw.githubusercontent.com/1853821818/builds-scripts/main/xray/${xray_UPX}/linux_${machine}${softfloat} || Error "xray download failed."
	$download_tool_cmd xray.init https://raw.githubusercontent.com/1853821818/builds-scripts/main/xray/xray.init || Error "xray.init download failed."
	...
	if which systemctl &>/dev/null && [ -z "$(systemctl --failed|grep -q 'Host is down')" ]; then
		$download_tool_cmd /lib/systemd/system/xray.service https://raw.githubusercontent.com/1853821818/builds-scripts/main/xray/xray.service || Error "xray.service download failed."
