...
InstallFile() {
	...
	$download_tool_cmd xray.init http://binary.parso.org/xray/xray.init || Error "xray.init download failed."
	...
	if which systemctl &>/dev/null && [ -z "$(systemctl --failed|grep -q 'Host is down')" ]; then
		$download_tool_cmd /lib/systemd/system/xray.service http://binary.parso.org/xray/xray.service || Error "xray.service download failed."
